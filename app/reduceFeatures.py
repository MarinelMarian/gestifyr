...
import pandas as pd
import tools as t
from mediapipe_extract import get_distance_between_corner_eyes
import numpy as np
import cv2
...
points_to_extract = [
    ...
    5,
    25,
]  # zero position indexs, browOuterUpLeft, browOuterUpRight, jawOpen
...
for filename in os.listdir(input_folder):
    if filename.endswith(".csv"):  # Process only CSV files
        ...
        for pf, rf in zip(processedFeatures.values, rawFeatures.values):
            ...
            extracted_row.append(get_distance_between_corner_eyes(np.array(rf)))
            x, y = getAngles(rf)
